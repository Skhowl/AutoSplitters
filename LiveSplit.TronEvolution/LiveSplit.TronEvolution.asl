...
state("gridgame", "1.01")
{
	...
	byte iAnonState : 0x1E89C18, 0, 0x908, 0xC, 0x382;
}
...
init
{
	...
	{
		default:
			version = "Unknown Version - Contact Skhowl";
...
start
{
	if (settings["start"] && old.iAnonState >= 0x9E && current.iAnonState == 0x96)
	{
...
split
{
	...
	{
		var BinkID = (short)(current.iLastBink*3+current.iBinkInfo);
		if (vars.LastBink != BinkID && settings.ContainsKey("bik_"+BinkID) && settings["bik_"+BinkID])
		{
			// vars.DebugMessage("*Split* BinkID: "+BinkID);
			vars.LastBink = BinkID;
			return true;
		}
	}
	if (settings["finish"] && old.iAnonState == 0x82 && current.iAnonState == 0x9E && (short)(current.iLastBink*3+current.iBinkInfo) == 0xD64)
	{
